...
加入這行 `DATABASE_URL="mysql://root@127.0.0.1:3309/personal-blog"`
...
回到 PlanetScale Dashboard 點擊確認，就完成 main 的部署了

接著執行 `pscale connect personal-blog main --port 3309`，把 localhost 連線指到 main 分支上

如此就把 table schema 的部分搞定了，下一步開始進行 API 開發

補充：如有需要先進行資料測試的話，可以執行 `npx prisma studio` 透過 UI 直接操作資料表
